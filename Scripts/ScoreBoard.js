...
function createThirdRow(){
    ...
    if(bowler.bowling.ballDetails.length!=0) {
        let oversDetail = object.overCount(bowler.bowling.ballDetails);
        let runningOverIndex = oversDetail.length - 1, count = 0;
        let runningOver = oversDetail[runningOverIndex];
        for (let i = 0; i < runningOver.ball.length; i++) {
           if (runningOver.ball[i].type.match(/WD/) || runningOver.ball[i].type == 'NB') {
...
function createCongoDiv(){
    ...
    div0.id = 'greenDiv';
    img1.src = 'https://pixabay.com/static/uploads/photo/2012/04/15/21/34/arrow-35386_960_720.png';
    img1.id = 'backArrow';
...
function secondInningConfirmDiv(){
    ...
    p.innerText = bowlingTeam.teamName +' need '+battingTeam.totalScore+' runs in '+runningMatch.matchOvers+' overs';
    p2.innerText = 'Required Run Rate '+object.getRequiredRunRate(battingTeam.totalScore,ball);
    button1.id = 'cancel';
    button2.id = 'ok';
            ...
            if(ballNo!=0) {
                ...
                if (lastBall.type == '') {
                    if (striker.playerName == lastBall.batsmanName) {
                        ...
                        let partnershipIndex = battingTeam.partnerShips.length-1;
                        battingTeam.partnerShips[partnershipIndex].player1Run = battingTeam.partnerShips[partnershipIndex].player1Run-lastBall.ballValue;
                        ...
                        battingTeam.partnerShips[partnershipIndex].player2Run = battingTeam.partnerShips[partnershipIndex].player2Run-lastBall.ballValue;
                    ...
                    let string = lastBall.ballValue+'WD';
                    console.log(string,lastBall.type);
                    if(lastBall.type == 'NB'){
                        battingTeam.partnershipScore = battingTeam.partnershipScore-1;
                        console.log('type Noball: '+battingTeam.partnershipScore);
                        ...
                        battingTeam.partnerShips[partnershipIndex].extra = battingTeam.partnerShips[partnershipIndex].extra-1;
                        console.log(battingTeam.partnerShips[partnershipIndex].extra);

                    }

                    else if(lastBall.type.match(/WD/)){
