...
function createPlayerDiv(team,player,type){
    console.log(team.tName,player,type)
    ...
    if(type == 'notPlayed'){
        console.log(type);
        li2.innerText = player.pName;
    }
    else{
        if(player.playerType=='playerNotPlaying'){
            li2.innerText = player.pName;
        }
        else{
            console.log(player.pName.playerName);
            li2.innerText = player.pName.playerName;
        }
        ...
        for (let i=0;i<team.matchNo.length;i++){
            ...
            if(games.matches[team.matchNo[i]].innings[0].battingTeam.players.length!==0){
                ...
                for(let i=0;i<playerNo;i++){
                    if(tempTeam.players[i].playerName ==player.pName.playerName){
...
function createPlayerDivList(team,players,type){
    ...
    for(let i=0;i<players.length;i++){
       ...
       div.appendChild( createPlayerDiv(team,players[i],type));
    }
    return div;
}
function playerArray(pName,playerType){
    this.pName = pName;
...
function createPlayerList(team,type){
    ...
    if(type =='played'){
        for(let i=0;i<notPlayedTeam.notPlayedTeams.length;i++){
            if(notPlayedTeam.notPlayedTeams[i].teamName == team.tName){
                for(let j=0;j<notPlayedTeam.notPlayedTeams[i].players.length;j++){
                   // let player = notPlayedTeam.notPlayedTeams[i].players[j].playerName;
                    let player = new playerArray(notPlayedTeam.notPlayedTeams[i].players[j].playerName,'playerNotPlaying');
                ...
                if(games.matches[team.matchNo[i]].innings[0].battingTeam.players.length!==0){
                    ...
                    for(let i=0;i<playerNo;i++){
                        ...
                        if(tempTeam.players[i].playerName!='Unknown'){
                            let player = new playerArray(tempTeam.players[i],'playing');
                            playersOfTeam.push(player);
                        }
    ...
    let div = createPlayerDivList(team,playersOfTeam,type);
    return div;
}
...
function playerAddBtnOnclick(team,type,button){
    ...
    document.getElementById('ok').onclick = () => {
        ...
        div22.remove();
        if(type =='notPlayed'){
            let player = new object.NotPlayedPlayer(newPlayerName,'notPlayedPlayer');
            for (let i = 0; i < notPlayedTeam.notPlayedTeams.length; i++) {
                if (notPlayedTeam.notPlayedTeams[i].teamName == team.teamName) {
...
function teamNameClicked(team,type) {
    ...
    if (type == 'notPlayed') {
        ...
        button.onclick = () => {
            ...
            if(notPlayedTeam.notPlayedTeams.length==0){
                let existingTeam = new object.NotPlayedTeam(team.tName, 'played');
                notPlayedTeam.notPlayedTeams.push(existingTeam);
                console.log('player add button clicked')
                playerAddBtnOnclick(team, type, button);
               // document.getElementById('center').appendChild(createPlayerList(team, type));
            }
            else{
                let exitInNotPlayedTeam;
                for (let i = 0; i < notPlayedTeam.notPlayedTeams.length; i++) {
                    if (notPlayedTeam.notPlayedTeams[i].teamName == team.tName) {
                        exitInNotPlayedTeam = true;
                        //playerAddBtnOnclick(team, type, button);
                    }
                    else{
                        exitInNotPlayedTeam = false;
                    }
                }
                if(exitInNotPlayedTeam){
                    playerAddBtnOnclick(team, type, button);
                }
                else {
                        let existingTeam = new object.NotPlayedTeam(team.tName, 'played');
    ...
    export function teamName(game) {
        ...
        let notPlayedTeamNameArray = [];
        if (notPlayedTeam.notPlayedTeams.length != 0) {
            for (let i = 0; i < notPlayedTeam.notPlayedTeams.length; i++) {
                notPlayedTeamNameArray.push(notPlayedTeam.notPlayedTeams[i].teamName);
            }
        }

        if (notPlayedTeam.notPlayedTeams.length != 0) {
            for (let i = 0; i < notPlayedTeamNameArray.length; i++) {
                for (let j = 0; j < teamInfos.length; j++) {
                    console.log(teamInfos[j].tName,notPlayedTeamNameArray[i])
                    if (notPlayedTeamNameArray[i] == teamInfos[j].tName){
                        notPlayedTeamNameArray.splice(i,1);
                    }

                }
            }
        }
        console.log(notPlayedTeamNameArray[0]);
        for (let i = 0; i < notPlayedTeamNameArray.length; i++) {
            for (let j = 0; j < notPlayedTeam.notPlayedTeams.length; j++) {
                if(notPlayedTeamNameArray[i] == notPlayedTeam.notPlayedTeams[j].teamName){
                //    console.log(notPlayedTeamNameArray[i],notPlayedTeam.notPlayedTeams[j])
                    div.appendChild(createDiv(notPlayedTeam.notPlayedTeams[j], 'notPlayed'));
                }
            }
        ...
        button.onclick = () => {
            ...
            document.getElementById('ok').onclick = () => {
                ...
                div.style.display = 'none';
                div22.remove();
                let newAddedTeam = new object.NotPlayedTeam(newTeamName, 'notPlayed');
